...
test("when event has valid dates and is in current day, isTodayEvent should return true", assert => {
    ...
    let event = {title:"Event", beginDate: date.beginOfCurrentDay()};
    let d = new Day();
...
//     let events = [{title:"Event", beginDate: date.beginOfCurrentDay()}];
//     let d = new Day();
//     d.fillcases(events);
...
// test("when findPosition return valid position for one event, this event should be in cases", assert => {
//     let stub = sinon.stub(Day.prototype, 'findPosition').callsFake(function fn(){
//         return 1;
//     });
...
//     console.log("Avant d.fillcases");
//     d.fillcases(events);
//     console.log("Après d.fillcases");
//     // console.log(d.cases);
...
test("when event already exists, it should not be in cases", assert => {
    ...
    let events = [{title:"Event", beginDate: date.beginOfCurrentDay()}, {title:"Event", beginDate: date.beginOfCurrentDay()}];
    let d = new Day();
    d.fillcases(events);

    assert.equal(d._cases.length, 1);
